...
async function deleteStory(evt) {
  ...
  // Remove the story from the story list and API
  await storyList.removeStory(currentUser, storyId);
}

// Attach the event listener for deleting a story
...
function putUserStoriesOnPage() {
  ...
  if (currentUser.ownStories.length === 0) {
    $ownStories.append("<h5>Add a story :)</h5>");
  } else {
...
function putFavoritesListOnPage() {
  ...
  if (currentUser.favorites.length === 0) {
    $favoritedStories.append("<h5>Add your favorite stories, we know you have some!</h5>");
  } else {
